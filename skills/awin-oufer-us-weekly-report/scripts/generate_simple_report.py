...
def section_1_new_publishers(doc, data):
    ...
    add_diagnosis(doc,
                  "新增结构良性 — Coupon 类占比 46.7% 符合 OUFER JEWELRY定位；"
...
def section_2_sales(doc, data):
    ...
    add_para(doc,
             "本节呈现 OUFER JEWELRY US（Awin 91941）的"
    ...
    add_screenshot(doc, shots["us_home"],
                   caption="图 2-1  US 站点 Awin 后台主屏（Advertiser 91941）")
...
def section_3_top_publishers(doc, data):
    ...
    add_screenshot(doc, shots["us_publishers"],
                   caption="图 3-1  US 站 Publisher Performance 全图（Awin 91941）")
...
""")
    ...
    parts.append('<p class="lede">本节呈现 OUFER JEWELRY US（Awin 91941）的'
    ...
    parts.append(_html_screenshot(shots["us_home"], "图 2-1  US 站点 Awin 后台主屏（Advertiser 91941）", embed_images))
    parts.append("</section>")
    ...
    parts.append(_html_screenshot(shots["us_publishers"], "图 3-1  US 站 Publisher Performance 全图（Awin 91941）", embed_images))
...
def build_markdown(data, out_path, screenshot_rel_dir="attachments"):
    ...
    parts.append("")
    parts.append("> 🔍 **诊断与建议** — 新增结构良性，Coupon 占比 46.7% 符合 OUFER JEWELRY定位。建议 7 日内对 30 家全部推送『欢迎邮件 + 产品 feed + banner + 测试佣金』四件套激活包，首单率目标 ≥ 60%。")
    parts.append("")
    ...
    parts.append("*图 2-1  US 站点 Awin 后台主屏（Advertiser 91941）*")
    parts.append("")
    ...
    parts.append("*图 3-1  US 站 Publisher Performance 全图（Awin 91941）*")
    parts.append("")
    ...
    parts.append("")
    parts.append("#affiliate #oufer #weekly-report")
    Path(out_path).write_text("\n".join(parts), encoding="utf-8")
