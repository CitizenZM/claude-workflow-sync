#!/usr/bin/env bash
# publish.sh — END-TO-END pipeline for Oufer simplified weekly report
#
# Steps:
#   1. Render DOCX + HTML + PDF + Obsidian Markdown from oufer_simple_data.json
...
BASE="Oufer-简化周报-${REPORT_DATE}"
echo "════════════════════ OUFER JEWELRY WEEKLY PUBLISH ════════════════════"
...
if [ $SKIP_GITHUB -eq 0 ]; then
  ...
  for img in us_home us_publishers_full; do
...
- Auto-published by awin-oufer-us-weekly-report skill"
