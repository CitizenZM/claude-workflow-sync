# Awin Oufer Weekly Report — Claude Code Skill

End-to-end automation for the **OUFER JEWELRY** affiliate marketing weekly report on Awin US (advertiser 91941). Operated by Cell Digital Technology Inc.
...
After bootstrap completes, edit `~/.claude/skills/awin-oufer-us-weekly-report/.env` with real Awin credentials and re-launch Claude Code. The skill auto-loads on next session.
...
| Trigger | Action |
|---|---|
| `@awin-oufer-us-weekly-report` | Mention to load skill into context |
| `生成 Oufer 简化周报并发布到 GitHub + Vercel` | Plain Chinese — Claude will pick the skill |
...
└── data/
    └── oufer_simple_data.example.json   # sanitized sample
```
...
Login + scrape Awin dashboards. Produces `output/oufer_simple_data.json`.

```bash
# Through Claude Code — invoke skill, Claude drives Playwright MCP
@awin-oufer-us-weekly-report
"抓取本周 Awin US Oufer 数据，更新 oufer_simple_data.json"
```
...
| GitHub private repo (`oufer-affiliate-weekly-reports`) | HTML + PDF + screenshots | Team archive | `github.com/CitizenZM/oufer-affiliate-weekly-reports` |
| Vercel deployment | Rendered HTML | Brand-side / external | `oufer-affiliate-weekly-reports-<hash>-barrons-projects-<id>.vercel.app` (SSO-walled) |
