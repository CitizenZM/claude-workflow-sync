// Impact Rockbros US bulk proposal script v1 — adapted from TCL v9
// Key mechanics:
...
//   - term selection: picks highest-commission "Performance" term (10–20%)
...
async (page) => {
  ...
  for (const card of cards) {
    ...
    // ── Term selection: pick "Performance" or highest-commission visible option ──
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const liCoords = await propFrame.evaluate(() => {
        ...
        // Prefer "Performance" term; fallback to any option with a % commission; fallback to opts[1] (skip placeholder [0])
        const perf = opts.find(l => l.textContent.toLowerCase().includes('performance'));
        const withPct = opts.find(l => /\d+%/.test(l.textContent));
        const best = perf || withPct || (opts.length > 1 ? opts[1] : null);
        if (!best) return null;
      ...
      const confirmed = await propFrame.evaluate(() => {
        ...
        return btns.some(b =>
          b.textContent.toLowerCase().includes('performance') ||
