...
async () => {
  ...
  // Read from window.__DEDUP — set by Sonnet, never embedded in this function body
  ...
  for (let i = 0; i < TARGET + 20; i++) {
    ...
    try {
      ...
      // Keep window.__DEDUP live for subsequent iterations within this page
      ...
      if (sl) {
        for (let o = 0; o < sl.options.length; o++) {
          if (sl.options[o].text.includes(COMM)) {
            sl.selectedIndex = o; sl.dispatchEvent(new Event('change', {bubbles: true})); break;
          }
        }
      }
      await sleep(300);
