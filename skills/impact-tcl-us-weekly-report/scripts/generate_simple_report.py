#!/usr/bin/env python3
"""Generate OUFER JEWELRY US Simplified Weekly Report — DOCX + HTML + PDF.
...
Outputs three formats from a single data source:
  ~/Downloads/Oufer-简化周报-YYYY-MM-DD.docx
  ~/Downloads/Oufer-简化周报-YYYY-MM-DD.html
  ~/Downloads/Oufer-简化周报-YYYY-MM-DD.pdf

Usage:
    python3 generate_simple_report.py --data output/oufer_simple_data.json \
        --out-dir ~/Downloads --date 2026-04-16

NOTE: Some narrative prose in this script still contains Rockbros-specific
numbers (e.g. "46.7%"). After Phase 1 scraping produces real Oufer data,
rewrite those paragraphs to match actual Oufer metrics.
"""
import argparse
import asyncio
import base64
...
def section_1_new_publishers(doc, data):
    ...
    brand_u = str(data.get("brand", "")).upper()
    top_type = by_type[0][0] if by_type else "—"
    top_type_share = by_type[0][2] if by_type else "—"
    second_type = by_type[1][0] if len(by_type) > 1 else "—"
    second_share = by_type[1][2] if len(by_type) > 1 else "—"
    add_para(doc,
             f"过去 30 天内 {brand_u} US 共新增 {total_count} 家 Publisher（US 站 {us_count} 家）。"
             f"渠道结构以 {top_type}（{top_type_share}）为主导，"
             f"{second_type} 占 {second_share}，符合 {brand_u} 品类典型流量结构。",
             size=10, color=GREY)
    ...
    add_diagnosis(doc,
                  f"新增结构良性 — {top_type} 占比 {top_type_share} 符合 {brand_u} 定位；"
                  f"建议 7 日内对 {total_count} 家全部推送『欢迎邮件 + 产品 feed + banner + 测试佣金』四件套激活包，"
...
def section_2_sales(doc, data):
    ...
    brand_u = str(data.get("brand", "")).upper()
    platform = data.get("platform", "Awin")
    program = data.get("program_id", "—")
    add_para(doc,
             f"本节呈现 {brand_u} US（{platform} {program}）的"
             f"周度与月度（MTD）核心绩效指标，并附 {platform} 后台原图截图作为佐证。",
             size=10, color=GREY)
    ...
    add_screenshot(doc, shots["us_home"],
                   caption=f"图 2-1  US 站点 {platform} 后台主屏（Program {program}）")


def section_3_top_publishers(doc, data):
    add_heading(doc, "三、Top Performing Publisher 排行", level=1)
    rows = data["section3_top_publishers"]
    n = len(rows)
    platform = data.get("platform", "Awin")
    program = data.get("program_id", "—")
    add_para(doc,
             f"下表为本期 Top {n} Publisher 绩效排行，按 GMV 贡献降序排列。"
             f"重点关注 Top 3 集中度与渠道结构（Content / Coupon / Loyalty / Newsletter）的均衡度，"
             "判断是否存在单点依赖风险与下一阶段补位空间。",
             size=10, color=GREY)

    add_heading(doc, f"3.1 Top {n} Publisher 明细", level=2)
    add_table(doc,
    ...
    add_screenshot(doc, shots["us_publishers"],
                   caption=f"图 3-1  US 站 Publisher Performance 全图（{platform} Program {program}）")
...
def section_4_emails(doc, data):
    ...
    prose = (
        ...
        f"个与 {brand_u} 直接相关的合作线程，按 4 类情报（寄样 / Paid Placement / 素材 / Onboarding）拆解。"
    ...
    ship_n = len(shipping)
    contract_n = len(contracts)
    ship_examples = " / ".join(r[0] for r in shipping[:2]) if shipping else "—"
    contract_examples = " / ".join(r[0] for r in contracts[:2]) if contracts else "—"
    add_diagnosis(doc,
                  f"邮件情报三类主诉求：(1) {ship_examples} 等伙伴要求寄样 — 直接影响首篇内容 / 评测排期；"
                  f"(2) {contract_examples} 等伙伴提出 Paid Placement / IO 采购 — 需品牌侧预算审批；"
                  "(3) 多家新伙伴要求品牌素材包 + 最新促销月历。"
                  f"建议品牌团队本周内完成：① 按 4.2 寄样地址簿执行 {ship_n} 单打包发货；② 对 4.3 Paid Placement 清单 {contract_n} 项逐一核算 ROI 并回复。",
...
def section_5_brand_actions(doc, data):
    ...
    actions = data.get("section5_brand_actions", [])
    p0 = sum(1 for r in actions if "P0" in r[0])
    p1 = sum(1 for r in actions if "P1" in r[0])
    p2 = sum(1 for r in actions if "P2" in r[0])
    p3 = sum(1 for r in actions if "P3" in r[0])
    add_diagnosis(doc,
                  f"上述 {len(actions)} 项品牌侧行动按优先级分为四档：⛳ 必做（P0，本周内 {p0} 项）、"
                  f"🚀 高优（P1，两周内 {p1} 项）、📅 计划（P2，本月内 {p2} 项）、🛡️ 长期（P3，本季度 {p3} 项）。"
                  "P0 项构成『救火三角』 — 缺一项就可能出现 Publisher 流失或行业负向口碑；"
                  "P1 项构成『增长三角』 — 完成后可释放结构性 GMV 增量。"
                  "Affiliate Marketing 团队将于每周一晨会同步进度看板。",
                  label="🎯 战略判断",
                  width_cm=LANDSCAPE_WIDTH_CM)


def section_footer(doc, data):
    doc.add_paragraph()
    p = doc.add_paragraph()
    ...
    p2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = p2.add_run("")
    operator = data.get("operator", "Cell Digital Technology Inc.")
    platform = data.get("platform", "Awin")
    set_run_zh(r,
               f"{operator}  |  联盟营销绩效办公室  |  数据来源：{platform} Advertiser Console",
...
""")
    ...
    top_type = _by_type[0][0] if _by_type else "—"
    top_type_share = _by_type[0][2] if _by_type else "—"
    second_type = _by_type[1][0] if len(_by_type) > 1 else "—"
    second_share = _by_type[1][2] if len(_by_type) > 1 else "—"
    parts.append(f'<p class="lede">过去 30 天内 {brand_u} US 共新增 {_total} 家 Publisher（US 站 {_us} 家）。'
                 f'渠道结构以 {top_type}（{top_type_share}）为主导，'
                 f'{second_type} 占比 {second_share}，构成 {brand_u} 当前阶段的典型流量结构。</p>')
    parts.append("<h3>1.1 Publisher 类型分布</h3>")
    parts.append(_html_table(["Publisher 类型", "新增家数", "占比"], summary["by_type"], accent_first_col=True))
    parts.append(f"<h3>1.2 {_total} 家新增 Publisher 明细</h3>")
    parts.append(_html_table(
        ["Publisher 名称", "区域", "网站 / 流量入口", "类型", "加入日期"],
        s["section1_new_publishers"], accent_first_col=True))
    parts.append(_html_diagnosis(
        f"新增结构良性 — {top_type} 占比 {top_type_share} 符合 {brand_u} 当前阶段的渠道定位；"
        f"建议 7 日内对 {_total} 家全部推送『欢迎邮件 + 产品 feed + banner + 测试佣金』四件套激活包，"
        f"把首单率目标定在 ≥ 60%（行业均值 35-40%）。"))
    parts.append("</section>")

    # Section 2
    parts.append('<section class="section">')
    parts.append("<h2>二、过去一周与本月销量数据</h2>")
    parts.append(f'<p class="lede">本节呈现 {brand_u} US（{platform} Program {program}）的'
                 f'周度与月度（MTD）核心绩效指标，并附 {platform} 后台原图截图作为佐证。</p>')
    ...
    parts.append(_html_screenshot(shots["us_home"], f"图 2-1  US 站点 {platform} 后台主屏（Program {program}）", embed_images))
    parts.append("</section>")

    # Section 3
    parts.append('<section class="section">')
    parts.append("<h2>三、Top Performing Publisher 排行</h2>")
    rows3 = s.get("section3_top_publishers", [])
    n = len(rows3)
    parts.append(f'<p class="lede">下表为本期 {brand_u} US 站 Top {n} Publisher 绩效排行，按 GMV 贡献降序排列。'
                 f'报告关注头部集中度与流量结构平衡，结合 4.x 节邮件情报，'
                 f'下一阶段将围绕 Top 3 续约、腰部赋能与新增 Publisher 激活三条主线推进。</p>')
    parts.append(f"<h3>3.1 Top {n} Publisher 明细</h3>")
    parts.append(_html_table(
        ["排名", "Publisher 名称", "区域", "GMV", "类型", "下一步动作"],
        rows3, accent_first_col=True))
    parts.append("<h3>3.2 后台明细截图</h3>")
    parts.append(_html_screenshot(shots["us_publishers"], f"图 3-1  US 站 Publisher Performance 全图（{platform} Program {program}）", embed_images))
    ...
    parts.append(
        ...
        f'共扫描 {_total if _total is not None else "—"} 个邮件线程，识别出 {_relevant} 个与 {brand_u} 直接相关的合作线程，'
    ...
    parts.append(_html_diagnosis(
        f"邮件情报三类主诉求："
        f"(1) {ship_examples} 等 Editorial 伙伴要求寄样（共 {ship_n} 单）— 直接影响首篇内容排期；"
        f"(2) {contract_examples} 等伙伴提出 Paid Placement / 合同合作（共 {contract_n} 单）— 需品牌侧预算审批；"
        f"(3) 所有新伙伴要求品牌素材包 + 最新 Coupon 月历。"
        f"建议品牌团队本周内完成：① 按 4.2 寄样地址簿执行 {ship_n} 单打包发货；② 对 4.3 Paid Placement 清单逐一核算 ROI 并回复。"))
    parts.append("</section>")
    ...
    actions5 = s.get("section5_brand_actions", [])
    p0 = sum(1 for r in actions5 if "P0" in str(r[0]))
    p1 = sum(1 for r in actions5 if "P1" in str(r[0]))
    p2 = sum(1 for r in actions5 if "P2" in str(r[0]))
    p3 = sum(1 for r in actions5 if "P3" in str(r[0]))
    parts.append(_html_diagnosis(
        f"上述 {len(actions5)} 项品牌侧行动按优先级分为四档：⛳ 必做（P0，本周内 {p0} 项）、"
        f"🚀 高优（P1，两周内 {p1} 项）、📅 计划（P2，本月内 {p2} 项）、🛡️ 长期（P3，本季度 {p3} 项）。"
        f"P0 项构成『救火三角』 — 缺一项就会出现 Publisher 流失或行业负向口碑；"
        f"P1 项构成『增长三角』 — 完成后预计可带来阶段性 GMV 增量。"
        f"Affiliate Marketing 团队将于每周一晨会同步进度看板。",
        label="🎯 战略判断"))
    parts.append("</section>")

    parts.append(f"""<footer>
  {_esc(operator)}  |  联盟营销绩效办公室  |  数据来源：{_esc(platform)} Advertiser Console
</footer>
</body></html>""")
...
def build_markdown(data, out_path, screenshot_rel_dir="attachments"):
    ...
    brand_slug = "".join(ch.lower() if ch.isalnum() else "-" for ch in str(brand)).strip("-") or "brand"
    platform = data.get("platform", "Awin")
    program = data.get("program_id", "—")
    operator = data.get("operator", "Cell Digital Technology Inc.")
    parts = []
    ...
    top_type = _by_type[0][0] if _by_type else "—"
    top_type_share = _by_type[0][2] if _by_type else "—"
    parts.append(f"过去 30 天 {brand_u} US 共新增 **{_total} 家** Publisher（US {_us}）。")
    parts.append("")
    parts.append("### 1.1 Publisher 类型分布")
    parts.append(_md_table(["Publisher 类型", "新增家数", "占比"], summary["by_type"]))
    parts.append("")
    parts.append(f"### 1.2 {_total} 家新增 Publisher 明细")
    parts.append(_md_table(
        ["Publisher 名称", "区域", "网站 / 流量入口", "类型", "加入日期"],
        s["section1_new_publishers"]))
    parts.append("")
    parts.append(f"> 🔍 **诊断与建议** — 新增结构良性，{top_type} 占比 {top_type_share} 符合 {brand_u} 当前阶段定位。建议 7 日内对 {_total} 家全部推送『欢迎邮件 + 产品 feed + banner + 测试佣金』四件套激活包，首单率目标 ≥ 60%。")
    parts.append("")
    ...
    parts.append(f"*图 2-1  US 站点 {platform} 后台主屏（Program {program}）*")
    parts.append("")
    ...
    parts.append(f"*图 3-1  US 站 Publisher Performance 全图（{platform} Program {program}）*")
    parts.append("")
    ...
    parts.append("")
    ship_n_md = len(shipping)
    contract_n_md = len(contracts)
    parts.append(f"> 🔍 **诊断与建议** — 邮件情报三类主诉求：Editorial 寄样（{ship_n_md} 单）、Paid Placement 预算审批（{contract_n_md} 单）、品牌素材包 + Coupon 月历。本周内按 4.2 执行 {ship_n_md} 单寄样，按 4.3 核算 Paid Placement ROI。")
    parts.append("")
    ...
    p0 = sum(1 for r in actions5 if "P0" in str(r[0]))
    p1 = sum(1 for r in actions5 if "P1" in str(r[0]))
    parts.append(f"> 🎯 **战略判断** — P0『救火三角』{p0} 项缺一项就会出现 Publisher 流失或负向口碑；P1『增长三角』{p1} 项完成可带来阶段性 GMV 增量。每周一晨会同步看板。")
    parts.append("")
    parts.append("---")
    parts.append(f"*{operator}  |  联盟营销绩效办公室  |  数据来源：{platform} Advertiser Console*")
    parts.append("")
    parts.append(f"#affiliate #{brand_slug} #weekly-report")
    Path(out_path).write_text("\n".join(parts), encoding="utf-8")
...
def main():
    ...
    parser.add_argument("--email-intel", default=None,
                        help="Optional: path to <brand>_email_intel.json produced by Gmail MCP scrape")
    ...
    base = f"{data.get('brand', 'Brand')}-简化周报-{args.date}"
