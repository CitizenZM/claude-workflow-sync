---
description: "Impact TCL US — Outreach + full publisher data collection. Sonnet login+setup+tab-loop → pre-built Haiku per tab. Usage: /impact-tcl-us-outreach-datacollection [count]"
model: sonnet
---

# Impact TCL US — Outreach + Data Collection Workflow
...
target_per_tab : COUNT (full count per tab, not capped)
login          : affiliate@celldigital.co / Celldigital2024*
scripts_dir    : ~/.claude/skills/impact-tcl-us-outreach-datacollection/scripts/
ledger         : ~/Documents/Obsidian Vault/01-Projects/Impact-TCL-US-Outreach-Ledger.md
intel_db       : ~/Documents/Obsidian Vault/01-Projects/Impact-TCL-US-Publisher-Intel.md
report         : ~/Documents/Obsidian Vault/01-Projects/Impact-TCL-US-Outreach-Report-DYNAMIC_DATE.md
obsidian       : ~/Documents/Obsidian Vault/01-Projects/Impact-TCL-US-Outreach.md
msg            : "Hi! We're reaching out on behalf of TCL — a global leader in TVs, smartphones & consumer electronics. We're offering an industry-leading 10% commission through our affiliate program. Join top publishers already earning with TCL. Review our proposal and reply with any questions — excited to partner with you!"
...
Navigate: `${ACTIVE_MCP}browser_navigate` → `https://app.impact.com`
...
```
If cards=0: wait 3s and check again. Print: `"✓ TCL US ready: {N} cards. Starting tab loop..."`
...
```bash
grep 'impact-48321' ~/Documents/Obsidian\ Vault/01-Projects/Impact-TCL-US-Outreach-Ledger.md \
...
Read `~/.claude/skills/impact-tcl-us-outreach-datacollection/scripts/bulk-proposal.js`
...
Write result to `/Users/xiaozuo/.claude/skills/impact-tcl-us-outreach-datacollection/scripts/tab_current.js`

Verify no remaining `%%` placeholders:
```bash
grep -c '%%' ~/.claude/skills/impact-tcl-us-outreach-datacollection/scripts/tab_current.js
```
...
Invoke Agent tool:
- `model`: `"haiku"`
- `description`: `"TCL US tab {tab_num} ({tab_name}) — {COUNT - session_sent} proposals"`
- `prompt`: PER-TAB HAIKU PROMPT below with `{tab_num}`, `{tab_name}`, `{ACTIVE_MCP}`, `{COUNT - session_sent}` filled in
...
```
{name}|{contact_email}|{YYYY-MM-DD}|impact-48321|{partner_id}|{status}|{partner_size}|{website}|{contact_name}
```
...
```
You are the Impact TCL US per-tab proposal agent (tab {tab_num}: {tab_name}).
MCP: {ACTIVE_MCP}
Browser is already logged in on the TCL discover page, tab: {tab_name}.
...
Read file: /Users/xiaozuo/.claude/skills/impact-tcl-us-outreach-datacollection/scripts/tab_current.js

Step 2 — Call browser_run_code:
{ACTIVE_MCP}browser_run_code with:
...
- Errors: {total_errors}
- Ledger total: {grep -c 'impact-48321' ledger} rows
...
Opus fires automatically on ANY of these triggers. Never stops the outer loop — always continues after fixing.

### Trigger 1: browser_run_code fails (Haiku returns total=0 + error)
Sonnet spawns Opus immediately:
```
Agent(model:"opus", description:"TCL US bug fix — tab {tab_num}", prompt:
"You are the TCL US auto-recovery supervisor. browser_run_code failed on tab {tab_num}.
MCP: {ACTIVE_MCP}
Error: {error_message}

Fix autonomously — no questions, no pausing:
1. {ACTIVE_MCP}browser_navigate to: {DISCOVER_URL}
2. Wait 3s, check cards: browser_evaluate → () => document.querySelectorAll('.iui-card').length
3. If cards > 0: re-run browser_run_code with tab_current.js content
4. Return result JSON or {\"total\":0,\"error\":\"opus_recovery_failed\",\"sent\":[],\"errors\":[\"unfixable\"]}
")
```
...
Sonnet runs directly:
```bash
bash ~/.claude/scripts/launch-impact-tcl-us-2.sh
```
Wait 5s. Re-detect ACTIVE_MCP. Re-navigate to current tab URL. Retry Haiku for that tab.
...
```
Agent(model:"opus", description:"TCL US tab {tab_num} — 2 consecutive failures", prompt:
...
```
Agent(model:"opus", description:"TCL US — zero proposals all tabs", prompt:
...
2. ALL browser calls use the auto-detected `${ACTIVE_MCP}` — no other namespace
