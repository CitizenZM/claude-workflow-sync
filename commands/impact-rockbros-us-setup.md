---
description: "Impact Rockbros US login + filter setup (Sonnet). Run before /impact-rockbros-us-outreach. Usage: /impact-rockbros-us-setup"
model: sonnet
---

## MODEL GATE — MANDATORY FIRST CHECK
This command REQUIRES model: **sonnet**. If you are running on Opus, STOP: "⛔ Wrong model. Run `/model sonnet` then re-run `/impact-rockbros-us-setup`." Do NOT proceed on Opus.

## MCP SERVER — MANDATORY
All browser tool calls MUST use `mcp__playwright-impact-rockbros-us__*` exclusively.
Profile: `~/.claude/browser-profiles/impact-rockbros-us` — port 9306.
...
## Step 1: Login
1. `mcp__playwright-impact-rockbros-us__browser_navigate` to `https://app.impact.com`
...
Report: "Impact Rockbros US Setup complete. {N} publishers on Content/Reviews tab. Run `/impact-rockbros-us-outreach` to start."
