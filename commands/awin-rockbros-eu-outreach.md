...
This command REQUIRES model: **haiku**. Before doing ANY work, check your current model. If you are running on Opus or Sonnet, STOP IMMEDIATELY and tell the user: "⛔ Wrong model. This command requires Haiku. Run `/model haiku` then re-run `/awin-rockbros-eu-outreach`." Do NOT proceed on the wrong model — it wastes 10-20x credits for identical work.
